/*
 * Accelerometer Interface to Tiva Hardware
 * Ammar 
 * 
 *
 * 2024
...
// Return acclerometer data for x,y,z respectively
void getAcclData(vector3_t* vector)
{
    ...
    vector->x = (fromAccl[2] << 8) | fromAccl[1]; // Return 16-bit acceleration readings.
    vector->y = (fromAccl[4] << 8) | fromAccl[3];
    vector->z = (fromAccl[6] << 8) | fromAccl[5];
}